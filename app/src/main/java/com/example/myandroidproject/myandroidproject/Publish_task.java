        ...
        private void sqlCon(Connection con1) throws SQLException {
            try { // 监控异常
                ...
                if (t.isEmpty() || p.isEmpty() || ec.isEmpty() || s.isEmpty()) {
                    ...
                    result = sta.executeUpdate("INSERT INTO task(user_id,title,content,sort,price,address,phone,create_time,flag)" +
                            " VALUES ('" + user_id + "','" + t + "','" + c + "','" + s + "','" + edit_price1 + "','" + ea + "','" + ec + "','" + create_time + "','"+"0')");
                }
                if (result > 0) {
                    Looper.prepare();
